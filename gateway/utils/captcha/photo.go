...
func NewCapt() *PhCaptcha {
	...
	driver := base64Captcha.NewDriverDigit(
		config.Get().PhotoCaptcha.Height,
		config.Get().PhotoCaptcha.Width,
		config.Get().PhotoCaptcha.Length,
		config.Get().PhotoCaptcha.MaxSkew,  // 倾斜角
		config.Get().PhotoCaptcha.DotCount, // 模糊点
	)
...
func (c *PhCaptcha) Set(id string, value string) error {
	_, err := redis.SetWithTime(redis.GetPhotoCaptchaKey(id), value, parse.Duration(config.Get().PhotoCaptcha.Expire))
	return err
}

func (c *PhCaptcha) Get(id string, clear bool) string {
	val, err := redis.Get(redis.GetPhotoCaptchaKey(id))
	if err != nil {
		return ""
	}

	if clear {
		redis.Del(redis.GetPhotoCaptchaKey(id))
	}
